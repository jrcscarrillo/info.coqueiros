...
function procesa_volante($db) {
    ...
    try {
        ...
        $estado .= " Error =>" . $e->getMessage() . " por " . $e->getLine() . ' factura ' . $_SESSION['invoice']['RefNumber'];
    }
    return $estado;
}
...
function actualiza_lotescab($db) {
    ...
    try {
        ...
        $stmt->bindParam(':VehicleID', 'n/a');
        $stmt->bindParam(':RouteID', 'n/a');
        $stmt->bindParam(':DriverID', 'n/a');
        $stmt->bindParam(':Responsable', 'n/a');
        $stmt->bindParam(':Status', $_SESSION['invoice']['Status']);
...
function adiciona_lotestrx($db) {
    ...
    if ($volante[0] === 'NTV') {
        ...
        try {
            ...
            $stmt = $db->prepare($sql);
            $stmt->bindParam(':TxnLineID', $_SESSION['invoicedetail']['TxnLineID']);
            $stmt->bindParam(':TimeCreated', $_SESSION['invoicedetail']['TimeCreated']);
            $stmt->bindParam(':TimeModified', $_SESSION['invoicedetail']['TimeModified']);
            $stmt->bindParam(':EditSequence', $_SESSION['invoicedetail']['EditSequence']);
            $stmt->bindParam(':TxnDate', $_SESSION['invoicedetail']['TxnDate']);
            $stmt->bindParam(':RefNumber', $_SESSION['invoicedetail']['RefNumber']);
            $stmt->bindParam(':ItemRef_ListID', $_SESSION['invoicedetail']['ItemRef_ListID']);
            $stmt->bindParam(':ItemRef_FullName', $_SESSION['invoicedetail']['ItemRef_FullName']);
            $stmt->bindParam(':Memo', 'n/a');
            $stmt->bindParam(':MemoTrx', 'n/a');
            $stmt->bindParam(':NumeroTrx', $_SESSION['invoicedetail']['EditSequence']);
            $stmt->bindParam(':FechaTrx', $_SESSION['invoicedetail']['TxnDate']);
            $stmt->bindParam(':TipoTrx', 'FACTURA');
            $stmt->bindParam(':OrigenTrx', $_SESSION['invoice']['ClassRef_ListID']);
