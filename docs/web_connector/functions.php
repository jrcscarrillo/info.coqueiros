...
function _quickbooks_item_add_request($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale){
    ...
    $arr = mysqli_fetch_assoc(mysqli_query("SELECT * FROM items WHERE id = " . (int) $ID));
...
function _quickbooks_sales_order_add_request($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale){
    ...
    $arr = mysqli_fetch_assoc(mysqli_query("SELECT * FROM customer_sales_order WHERE id = ".(int)$ID));
            $sales_items        = mysqli_query("SELECT * FROM sales_order_items WHERE sales_order_id = ".(int)$ID);
            $sales_items_arr    = array();
            
            while($row = mysqli_fetch_assoc($sales_items)) {
                $sales_items_arr[$row['item_id']]    = $row;
                $sales_item_ids[]                    = $row['item_id'];
            }
           
            $customer   = mysqli_fetch_assoc(mysqli_query("SELECT * FROM my_customer_table WHERE id = ".(int)$arr['customer_id']));
            $items      = mysqli_query("SELECT * FROM items WHERE id IN (".implode(',',$sales_item_ids).")");
...
<QBXML>
    <QBXMLMsgsRq onError="continueOnError">
        <SalesOrderAddRq>
            ...
            fwrite($myfile, $customer['fullname'] . " " . $item_xml);
    return $xml;
}
...
function _quickbooks_sales_order_add_response($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents){	
	mysqli_query("
		...
		SET 
			quickbooks_listid = '" . mysqli_real_escape_string($idents['ListID']) . "',
                        reference_number = '". mysqli_real_escape_string($idents['RefNumber']) ."',
			quickbooks_editsequence = '" . mysqli_real_escape_string($idents['EditSequence']) . "'
		WHERE 
			id = " . (int) $ID);
}
