...
function send_mail($sales_order_object){
    ...
    if(isset($sales_order_object['cc'])) {
        $mail_obj['to_email']   = 'jrcscarrillov@gmail.com,'.$sales_order_object['cc'];
    } else {
        $mail_obj['to_email']   = 'jrcscarrillov@gmail.com';
    }
...
if($_REQUEST){
    ...
    switch ($action){
        ...
        case 'customer_list':
            ...
            $array      = mysqli_query($query);
            
            while($row = mysqli_fetch_assoc($array)) {
                $sale_tax_code  = mysqli_fetch_assoc(mysqli_query("SELECT * FROM tax_codes WHERE quickbooks_listid = '{$row['sales_tax_code_ref_listid']}'"));
                $item_tax_code  = mysqli_fetch_assoc(mysqli_query("SELECT * FROM sale_tax_items WHERE quickbooks_listid = '{$row['item_sales_tax_ref_listid']}'"));               
        ...
        case 'customer_add':
            ...
            if(!isset($_POST) && empty($_POST)){
                ...
                mysqli_query(" INSERT INTO my_customer_table (name,fname,lname,salutation,address,city,state,zipcode,country,email)                     
                
                VALUES (
                        '" . mysqli_real_escape_string($_REQUEST['name']) . "', 
                        '" . mysqli_real_escape_string($_REQUEST['fname']) . "', 
                        '" . mysqli_real_escape_string($_REQUEST['lname']) . "',
                        '" . mysqli_real_escape_string($_REQUEST['salutation']) . "', 
                        '" . mysqli_real_escape_string($_REQUEST['address']) . "', 
                        '" . mysqli_real_escape_string($_REQUEST['city']) . "',
                        '" . mysqli_real_escape_string($_REQUEST['state']) . "', 
                        '" . mysqli_real_escape_string($_REQUEST['zipcode']) . "', 
                        '" . mysqli_real_escape_string($_REQUEST['country']) . "',
                        '" . mysqli_real_escape_string($_REQUEST['email']) . "'
                )");
                ...
                $customer_details   = mysqli_fetch_assoc(mysqli_query("SELECT * FROM my_customer_table WHERE id = {$id}"));
                $sale_tax_code      = mysqli_fetch_assoc(mysqli_query("SELECT * FROM tax_codes WHERE quickbooks_listid = '{$customer_details['sales_tax_code_ref_listid']}'"));
                $item_tax_code      = mysqli_fetch_assoc(mysqli_query("SELECT * FROM sale_tax_items WHERE quickbooks_listid = '{$customer_details['item_sales_tax_ref_listid']}'"));                               
                $customer_details['item_tax_code_quickbooks_listid']        = $sale_tax_code['quickbooks_listid'];
                $customer_details['item_tax_code_name']                     = $sale_tax_code['name'];
                $customer_details['item_tax_code_description']              = $sale_tax_code['description'];
                $customer_details['customer_tax_code_quickbooks_listid']    = $item_tax_code['quickbooks_listid'];
        ...
        case 'customer_edit':            
            ...
            if(isset($_POST['name'])){
                ...
                $customer_details   = mysqli_fetch_assoc(mysqli_query("SELECT * FROM my_customer_table WHERE id = {$_REQUEST['id']}"));
                $sale_tax_code      = mysqli_fetch_assoc(mysqli_query("SELECT * FROM tax_codes WHERE quickbooks_listid = '{$customer_details['sales_tax_code_ref_listid']}'"));
                $item_tax_code      = mysqli_fetch_assoc(mysqli_query("SELECT * FROM sale_tax_items WHERE quickbooks_listid = '{$customer_details['item_sales_tax_ref_listid']}'"));                               
                $customer_details['item_tax_code_quickbooks_listid']        = $sale_tax_code['quickbooks_listid'];
                $customer_details['item_tax_code_name']                     = $sale_tax_code['name'];
        ...
        case 'item_add':            
            ...
            if(!isset($_POST) && empty($_POST)){
                ...
                mysqli_query(" INSERT INTO items (name,amount,description) 
                    
                VALUES (
                        '" . mysqli_real_escape_string($_REQUEST['name']) . "', 
                        '" . mysqli_real_escape_string($_REQUEST['amount']) . "', 
                        '" . mysqli_real_escape_string($_REQUEST['description']) . "'                        
                )");
                ...
                $customer_details   = mysqli_fetch_assoc(mysqli_query("SELECT * FROM items WHERE id = {$id}"));
        ...
        case 'item_list':            
            ...
            $array              = mysqli_query("SELECT * FROM items");
            $unit_of_measure    = array();
            while($row  = mysqli_fetch_assoc($array)) {
                $unit_of_measure    = mysqli_fetch_assoc(mysqli_query("SELECT * FROM unit_of_meaures WHERE quickbooks_listid = '{$row['unit_of_measure_set_ref_listid']}'"));
                unset($unit_of_measure['id']);
                $items[]            = is_array($unit_of_measure)?array_merge($row,$unit_of_measure):$row;                
            }
            
            $units      = mysqli_query("SELECT * FROM unit_of_meaures");
        ...
        case 'sales_order_list':            
            ...
            $array              = mysqli_query($sales_order_query);
            $item_detail        = mysqli_query($item_query);
            
            while($row = mysqli_fetch_assoc($item_detail)) {                
                $unit_of_measure    = mysqli_fetch_assoc(mysqli_query("SELECT * FROM unit_of_meaures WHERE quickbooks_listid = '{$row['unit_of_measure_set_ref_listid']}'"));
                unset($unit_of_measure['id']);
                $items[$row['id']]  = is_array($unit_of_measure)?array_merge($row,$unit_of_measure):$row;   
            }
            $item_tax_code  = mysqli_query("SELECT * FROM tax_codes");
            
            while($row = mysqli_fetch_assoc($item_tax_code)){
                $item_tax[$row['quickbooks_listid']] =  $row['name'];
            }
            
            while($row = mysqli_fetch_assoc($array)) {                 
                $sales_order_items          = mysqli_query("SELECT * FROM sales_order_items where sales_order_id = {$row['id']}");
                $sale_order_items_details   = array();
                $i = 0;
                while($sales_item = mysqli_fetch_assoc($sales_order_items)) {   
        ...
        case 'sales_order_add':            
            ...
            if(!isset($_POST) && empty($_POST)){
                ...
                mysqli_query(" INSERT INTO customer_sales_order (customer_id,shipping_address,address,city,state,country,zipcode,date,tax_type,signature,note) 
                
                VALUES (
                        '" . mysqli_real_escape_string($_REQUEST['customer_id']) . "',                         
                        '" . mysqli_real_escape_string($_REQUEST['shipping_address']) . "',
                        '" . mysqli_real_escape_string($address) . "',
                        '" . mysqli_real_escape_string($city) . "',
                        '" . mysqli_real_escape_string($state) . "',
                        '" . mysqli_real_escape_string($country) . "',
                        '" . mysqli_real_escape_string($zipcode) . "',
                        '" . date('Y-m-d',time()). "',
                        '" . mysqli_real_escape_string($_REQUEST['tax_type']) . "',
                        '" . mysqli_real_escape_string($_REQUEST['signature']) . "',
                        '" . mysqli_real_escape_string($_REQUEST['note']) . "'                        
                )");
                ...
                $tax_codes      = mysqli_query("SELECT * FROM tax_codes");
            
                while($row = mysqli_fetch_assoc($tax_codes)){
                    $item_tax[$row['quickbooks_listid']] =  $row['name'];
                }
                ...
                for($i=0;$i<count($_REQUEST['item_id']);$i++){                    
                    if(!empty($_REQUEST['quantity'][$i]) || ($_REQUEST['quantity'][$i] != 0)){
                        ...
                        $queryStr[] = "(
                            '" .$id. "',
                            '" . mysqli_real_escape_string($_REQUEST['item_id'][$i]) . "', 
                            '" . mysqli_real_escape_string($_REQUEST['quantity'][$i]) . "',
                            '" . mysqli_real_escape_string($_REQUEST['description'][$i]) . "',
                            '" . mysqli_real_escape_string($_REQUEST['unit_of_measure'][$i]) . "',
                            '" . mysqli_real_escape_string($_REQUEST['tx'][$i]) . "'
                        )";
                    }                    
                }
                ...
                $items                 = array();
                $sales_order_details   = mysqli_fetch_assoc(mysqli_query("SELECT * FROM customer_sales_order WHERE id = {$id}"));
                $customer_detail       = mysqli_fetch_assoc(mysqli_query("SELECT * FROM my_customer_table WHERE id = {$_REQUEST['customer_id']}"));
                $sale_tax_code         = mysqli_fetch_assoc(mysqli_query("SELECT * FROM tax_codes WHERE quickbooks_listid = '{$customer_detail['sales_tax_code_ref_listid']}'"));
                $item_tax_code         = mysqli_fetch_assoc(mysqli_query("SELECT * FROM sale_tax_items WHERE quickbooks_listid = '{$customer_detail['item_sales_tax_ref_listid']}'"));                               
                ...
                $item_detail    = mysqli_query("SELECT * FROM items WHERE id IN (".implode(',',$item_ids).")");
                ...
                $customer_detail       = mysqli_query("SELECT * FROM my_customer_table");
                $item_detail           = mysqli_query("SELECT * FROM items");
                
                while($row = mysqli_fetch_assoc($customer_detail)) {
                    $sale_tax_code  = mysqli_fetch_assoc(mysqli_query("SELECT * FROM tax_codes WHERE quickbooks_listid = '{$row['sales_tax_code_ref_listid']}'"));
                    $item_tax_code  = mysqli_fetch_assoc(mysqli_query("SELECT * FROM sale_tax_items WHERE quickbooks_listid = '{$row['item_sales_tax_ref_listid']}'"));               
                    $customer_details   = $row;
                    $customer_details['item_tax_code_quickbooks_listid']        = $sale_tax_code['quickbooks_listid'];
            ...
            if(isset($_REQUEST['customer_id']) && ($_REQUEST['customer_id'] != 0)){
                ...
                mysqli_query("
                    UPDATE customer_sales_order SET
                        customer_id         = '".mysqli_real_escape_string($_POST['customer_id'])."',                        
                        shipping_address    = '".mysqli_real_escape_string($_POST['shipping_address'])."',
...
//                $Queue->enqueue(QUICKBOOKS_MOD_INVOICE, $id);
                ...
                $items                 = array();
                $sales_order_details   = mysqli_fetch_assoc(mysqli_query("SELECT * FROM customer_sales_order WHERE id = {$_REQUEST['id']}"));
                $customer_detail       = mysqli_fetch_assoc(mysqli_query("SELECT * FROM my_customer_table WHERE id = {$_REQUEST['customer_id']}"));
                $sale_tax_code         = mysqli_fetch_assoc(mysqli_query("SELECT * FROM tax_codes WHERE quickbooks_listid = '{$customer_detail['sales_tax_code_ref_listid']}'"));
                $item_tax_code         = mysqli_fetch_assoc(mysqli_query("SELECT * FROM sale_tax_items WHERE quickbooks_listid = '{$customer_detail['item_sales_tax_ref_listid']}'"));                               
                $customer_detail['item_tax_code_quickbooks_listid']        = $sale_tax_code['quickbooks_listid'];
                ...
                $customer_detail    = mysqli_query("SELECT * FROM customer_sales_order");
                $item_detail        = mysqli_query("SELECT * FROM items");
        ...
        case 'user_add':
            ...
            if(!isset($_POST) && empty($_POST)){
                ...
                $user_details   = mysqli_fetch_assoc(mysqli_query("SELECT * FROM tabusers WHERE email_id = '{$_REQUEST['email_id']}'"));
                ...
                mysqli_query(" INSERT INTO tabusers (email_id,password,first_name,last_name,created) 
                    
                VALUES (
                        '" . mysqli_real_escape_string($_REQUEST['email_id']) . "', 
                        '" . md5($_REQUEST['password']) . "', 
                        '" . mysqli_real_escape_string($_REQUEST['first_name']) . "',
                        '" . mysqli_real_escape_string($_REQUEST['last_name']) . "',   
                ...
                $user_details   = mysqli_fetch_assoc(mysqli_query("SELECT * FROM tabusers WHERE id = {$id}"));
        ...
        case 'user_edit':
            ...
            if(isset($_POST['first_name'])){
                ...
                $query    .= " WHERE id    = ".mysqli_real_escape_string($_POST['id']);
                
                mysqli_query($query);
                
                $user   = mysqli_fetch_assoc(mysqli_query("SELECT * FROM tabusers WHERE id = {$_REQUEST['id']}"));
        ...
        case 'user_login':
             ...
             if(!isset($_POST) && empty($_POST)){
                ...
                $user_details   = mysqli_fetch_assoc(mysqli_query("SELECT * FROM tabusers WHERE email_id = '{$_REQUEST['email_id']}' AND password ='".md5($_REQUEST['password'])."'"));
        ...
        case 'generate_report':
            ...
            $customer_query = mysqli_query($customer_query);
            $customers      = array();
        ...
        case 'import_customer':
            ...
            if (($handle = fopen($file, "r")) !== FALSE) {
                 while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {                        
                     if($i > 1){                        
                         $queryStr[]  = "(
                                '" . mysqli_real_escape_string($data[4]) . "', 
                                '" . mysqli_real_escape_string($data[5]) . "', 
                                '" . mysqli_real_escape_string($data[12]) . "',
                                '" . mysqli_real_escape_string($data[14]) . "', 
                                '" . mysqli_real_escape_string($data[11]) . "', 
                                '" . mysqli_real_escape_string($data[15]) . "',
                                '" . mysqli_real_escape_string($data[20]) . "', 
                                '" . mysqli_real_escape_string($data[21]) . "', 
                                '" . mysqli_real_escape_string($data[24]) . "',
                                '" . mysqli_real_escape_string($data[25]) . "', 
                                '" . mysqli_real_escape_string($data[0]) . "',
                                '" . mysqli_real_escape_string($data[3]) . "',
                                '" . mysqli_real_escape_string($data[59]) . "',
                                '" . mysqli_real_escape_string($data[60]) . "',
                                '" . mysqli_real_escape_string($data[65]) . "',
                                '" . mysqli_real_escape_string($data[66]) . "',
                                '" . mysqli_real_escape_string($data[67]) . "',
                                '" . mysqli_real_escape_string($data[68]) . "',
                                '" . mysqli_real_escape_string($data[69]) . "',
                                '" . mysqli_real_escape_string($data[70]) . "'
                        )"; 
                     }
                     $i++;
                 }               
                 ...
                 while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {                            
                     if($i > 0){                        
                         $queryStr[]  = "(
                                ...
                                '" . mysqli_real_escape_string($data[4]) . "',
                                '" . mysqli_real_escape_string($data[5]) . "', 
                                '" . mysqli_real_escape_string($data[6]) . "', 
                                '" . mysqli_real_escape_string($data[7]) . "',
                                '" . mysqli_real_escape_string($data[8]) . "', 
                                '" . mysqli_real_escape_string($data[9]) . "', 
                                '" . mysqli_real_escape_string($data[10]) . "',
                                '" . mysqli_real_escape_string($data[11]) . "',
                                '" . mysqli_real_escape_string($data[12]) . "',
                                '" . mysqli_real_escape_string($data[13]) . "',
                                '" . mysqli_real_escape_string($data[14]) . "',
                                '" . mysqli_real_escape_string($data[15]) . "'
                        )"; 
                     }
        ...
        case 'send_mail':
            ...
            if(isset($_REQUEST['id']) && !empty($_REQUEST['id'])){
                ...
                $sales_order_details    = mysqli_fetch_assoc(mysqli_query("SELECT * FROM customer_sales_order WHERE id = {$_REQUEST['id']}"));
                $customer_detail        = mysqli_fetch_assoc(mysqli_query("SELECT * FROM my_customer_table WHERE id = {$sales_order_details['customer_id']}"));
                $sale_tax_code          = mysqli_fetch_assoc(mysqli_query("SELECT * FROM tax_codes WHERE quickbooks_listid = '{$customer_detail['sales_tax_code_ref_listid']}'"));                
                $item_tax_code          = mysqli_fetch_assoc(mysqli_query("SELECT * FROM sale_tax_items WHERE quickbooks_listid = '{$customer_detail['item_sales_tax_ref_listid']}'"));
                $sales_order_item_list  = mysqli_query("SELECT * FROM sales_order_items WHERE sales_order_id = {$_REQUEST['id']}"); 
                $item_ids               = array();
                ...
                while($row = mysqli_fetch_assoc($sales_order_item_list)) {  
