...
define('QB_QUICKBOOKS_MAX_RETURNED', 3025);
...
try {
    ...
    $estado .= 'ERROR JC!!! ' . $estado . $e->getMessage();
}
$fecha = date("Y-m-d", strtotime($registro['otrosDesde']));
define('FECHAMODIFICACION', $fecha);
fwrite($myfile, 'fechas : ' . FECHAMODIFICACION . ' estado ' . $estado . '\r\n');
fclose($myfile);
$stmt = null;
$db = null;

$map = array(
   QUICKBOOKS_IMPORT_VENDOR => array('_pregunta', '_respuesta'),
);
$errmap = array(
...
function _pregunta($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale) {
...
function _respuesta($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents) {
    ...
    $doc = new DOMDocument();
    $myfile1 = fopen("vendor.xml", "w") or die("Unable to open file!");
    fwrite($myfile1, $xml);
    fclose($myfile1);
    $existe = "JRCS";
    $doc->load('vendor.xml');
    ...
    $proveedor = $doc->getElementsByTagName('VendorRet');
    if ($proveedor->length == 0) {
        $estado = 'no esta encontrando el nodo !!!!!';
    }
    foreach ($proveedor as $V) {
        genLimpia_vendor();
        gentraverse_vendor($V);
        $existe = buscaIgual_vendor($db);
        if ($existe == "OK") {
            quitaslashes_vendor();

            $estado = adiciona_vendor($db);

            fwrite($myfile, "Nuevo proveedor " . $_SESSION['vendor']['ListID'] . $estado . " \r\n");
        } elseif ($existe == "ACTUALIZA") {
            quitaslashes_vendor();

            $estado = update_vendor($db);

            fwrite($myfile, "SI Existe proveedor " . $_SESSION['vendor']['ListID'] . $estado . " \r\n");
        } else {
            fwrite($myfile, "Existe errores al acceder a DB " . $existe . ' ' . $_SESSION['vendor']['ListID'] . " \r\n");
        }
        $k++;
    }
    fwrite($myfile, "Se revisaron del XML " . $k . ' ' . $estado . " vendor ret");
    fclose($myfile);
...
function _inicio() {
    $myfile = fopen("newfile.txt", "w") or die("Unable to open file!");
    fwrite($myfile, "Inicio del proceso sincroniza proveedores: ");
    fclose($myfile);
}
...
function genLimpia_vendor() {
    ...
    $_SESSION['vendor']['IsSalesTaxAgency'] = ' ';
  ...
  foreach($child as $item) {
    if ($item->nodeType == XML_TEXT_NODE) {
      if (strlen(trim($item->nodeValue))) {
          $array[$item->nodeName] = trim($item->nodeValue);
      }
    }
    else if ($item->nodeType == XML_ELEMENT_NODE) parsedoc($item);
  }
