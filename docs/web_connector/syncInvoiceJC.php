    ...
    foreach ($factura as $uno) {
        ...
        foreach ($params2 as $p) {
            ...
            foreach ($params3 as $p2) {
                ...
                if ($params3->item($j)->nodeValue == "Facturacion Electronica Tipo") {
                    $_SESSION['invoice']['CustomField2'] = $params4->item($j)->nodeValue;
                }                
                $j++;
            }
            $i++;
        ...
        $sihay = buscaIgual_lotescab($db);
        $estado = 'INIT';
        $status = 'INIT';
        $_SESSION['volante'] = 0;
        quitaslashes_invoice();
        if ($sihay == "OK") {
            $estado = adiciona_lotescab($db);
        } elseif ($existe == "ACTUALIZA") {
            if ($_SESSION['invoice']['Memo'] === 'VOID:') {
                $estado = delete_lotescab($db);
            } else {
                $estado = actualiza_lotescab($db);
                $status = delete_lotestrx($db);
            }
            }
        if ($existe == "OK") {
            ...
            if ($_SESSION['invoice']['Memo'] === 'VOID:') {
                $estado = delete_invoice($db);
            } else {
                $estado = actualiza_invoice($db);
                $status = delete_invoicedetail($db);
            }
        }
        fwrite($myfile, "INICIO " . $estado . " " . $status . " " . $existe . "\r\n");

        $detalle = $factura->item($k)->getElementsByTagName('InvoiceLineRet');
        foreach ($detalle as $uno) {
            genLimpia_invoicedetail();
            gentraverse_invoicedetail($uno, $k);
            $estado = adiciona_invoicedetail($db);
            $status = adiciona_lotestrx($db);
            fwrite($myfile, " detail " . $estado . " lotes " . $status);
        }
        $k++;
        if($_SESSION['volante'] > 0){
            $sql = 'UPDATE lotestrx SET NumeroTrx = :numero WHERE IDKEY = :clave;';
            $stmt = $db->prepare($sql);
            $stmt->bindParam(':numero', $_SESSION['volante']);
            $stmt->bindParam(':clave', $_SESSION['invoce']['TxnID']);
            $stmt->execute();
        }
    }
    fclose($myfile);
...
function genLimpia_invoicedetail() {
    ...
    $_SESSION['invoicedetail']['ServiceDate'] = '2018-01-01';
...
function gentraverse_invoicedetail($node, $k) {
...
function buscaIgual_invoice($db) {
    $estado = "Procesando " . $_SESSION['invoice']['TxnID'];
    try {
        ...
        if (!$registro) {
            ...
            if ($registro['TxnID'] === $_SESSION['invoice']['TxnID']) {
                $estado = "ACTUALIZA";
                $_SESSION['invoice']['CustomField10'] = $registro['CustomField10'];
                $_SESSION['invoice']['CustomField15'] = $registro['CustomField15'];
            }
        }
    } catch (PDOException $e) {
        $estado .= " Error =>" . $e->getMessage() . " por " . $e->getLine();
    }
    return $estado;
}

function buscaIgual_lotescab($db) {
    $estado = "Procesando " . $_SESSION['invoice']['TxnID'];
    try {
            ...
            switch ($nivel1->nodeName) {
                ...
                case 'ShipAddress':
                    foreach ($nivel1->childNodes as $nivel2) {
                        switch ($nivel2->nodeName) {
                            case 'Addr1':
                                $_SESSION['invoice']['ShipAddress_Addr1'] = $nivel2->nodeValue;
                                break;
                            case 'Addr2':
                                $_SESSION['invoice']['ShipAddress_Addr2'] = $nivel2->nodeValue;
                                break;
                            case 'Addr3':
                                $_SESSION['invoice']['ShipAddress_Addr3'] = $nivel2->nodeValue;
                                break;
                            case 'Addr4':
                                $_SESSION['invoice']['ShipAddress_Addr4'] = $nivel2->nodeValue;
                                break;
                            case 'Addr5':
                                $_SESSION['invoice']['ShipAddress_Addr5'] = $nivel2->nodeValue;
                                break;
                            case 'City':
                                $_SESSION['invoice']['ShipAddress_City'] = $nivel2->nodeValue;
                                break;
                            case 'State':
                                $_SESSION['invoice']['ShipAddress_State'] = $nivel2->nodeValue;
                                break;
                            case 'PostalCode':
                                $_SESSION['invoice']['ShipAddress_PostalCode'] = $nivel2->nodeValue;
                                break;
                            case 'Country':
                                $_SESSION['invoice']['ShipAddress_Country'] = $nivel2->nodeValue;
                                break;
                            case 'Note':
                                $_SESSION['invoice']['ShipAddress_Note'] = $nivel2->nodeValue;
                                break;
                        }
                    }
                    break;
...
function genLimpia_invoice() {
    ...
    $_SESSION['invoice']['SalesTaxPercentage'] = 0;
...
function actualiza_invoice($db) {
    $estado = 'Procesando actualiza invoice ' . $_SESSION['invoice']['TxnNumber'];
    ...
    try {
        ...
        $estado .= 'ERROR JC!!! ' . $e->getMessage() . $_SESSION['invoice']['TxnID'] . '<br>';
    }
    return $estado;
}

function adiciona_invoice($db) {
    $estado = 'Procesando adiciona_invoice ' . $_SESSION['invoice']['TxnID'];
    try {
...
function adiciona_invoicedetail($db) {
    $estado = "Procesando adiciona_invoicedetail " . $_SESSION['invoicedetail']['ItemRef_FullName'];
    try {
        ...
        $estado .= " error => " . $e->getMessage() . " aqui " . $e->getLine() . $_SESSION['invoicedetail']['TxnLineID'];
    }
    return $estado;
}

function delete_invoice($db) {
    $estado = 'INIT';
    try {
        $sql = 'DELETE FROM invoice WHERE TxnID = :clave ';
        $stmt = $db->prepare($sql);
        $stmt->bindParam(':clave', $_SESSION['invoice']['TxnID']);
        $stmt->execute();
    } catch (PDOException $e) {
...
function delete_invoicedetail($db) {
    $estado = 'INIT';
    try {
        $sql = 'DELETE FROM invoicelinedetail WHERE IDKEY = :clave ';
        $stmt = $db->prepare($sql);
        $stmt->bindParam(':clave', $_SESSION['invoice']['TxnID']);
        $stmt->execute();
    } catch (PDOException $e) {
...
function adiciona_lotescab($db) {
    $estado = 'init';
    try {
        $sql = 'INSERT INTO lotestrxcab (TxnID, TimeCreated, TimeModified, '
           . 'EditSequence, TxnDate, RefType, RefNumber, OrigenID, DestinoID, '
           . 'Status, Estado) '
           . 'VALUES ( :TxnID, :TimeCreated, :TimeModified, :EditSequence, '
           . ':TxnDate, :RefType, :RefNumber, :OrigenID, :DestinoID, '
           . ':Status, :Estado)';
        $cerrado = 'CERRADO';
        $factura = 'FACTURA';
        $stmt = $db->prepare($sql);
        ...
        $stmt->bindParam(':EditSequence', $_SESSION['invoice']['EditSequence']);
        $stmt->bindParam(':TxnDate', $_SESSION['invoice']['TxnDate']);
        $stmt->bindParam(':RefType', $factura);
        $stmt->bindParam(':RefNumber', $_SESSION['invoice']['RefNumber']);
        $stmt->bindParam(':OrigenID', $_SESSION['invoice']['ClassRef_ListID']);
        $stmt->bindParam(':DestinoID', $_SESSION['invoice']['CustomerRef_ListID']);
        $stmt->bindParam(':Status', $_SESSION['invoice']['Status']);
        $stmt->bindParam(':Estado', $cerrado);
        $stmt->execute();
    } catch (PDOException $e) {
        $estado .= 'ERROR JC!!! ' . $e->getMessage() . $_SESSION['invoice']['TxnID'] . '<br>';
    }
...
function actualiza_lotescab($db) {
    $estado = 'Procesando actualiza invoice ' . $_SESSION['invoice']['TxnNumber'];
    $cerrado = 'CERRADO';
    $factura = 'FACTURA';
    try {
        $sql = 'UPDATE lotestrxcab SET TimeCreated=:TimeCreated, TimeModified=:TimeModified, '
           . 'EditSequence=:EditSequence, TxnDate=:TxnDate, RefType=:RefType, RefNumber=:RefNumber, '
           . 'OrigenID=:OrigenID, DestinoID=:DestinoID, Status=:Status, Estado=:Estado WHERE TxnID = :clave;';
        $stmt = $db->prepare($sql);
        $stmt->bindParam(':TimeCreated', $_SESSION['invoice']['TimeCreated']);
        $stmt->bindParam(':TimeModified', $_SESSION['invoice']['TimeModified']);
        $stmt->bindParam(':EditSequence', $_SESSION['invoice']['EditSequence']);
...
function delete_lotescab($db) {
    $estado = 'INIT';
    try {
        $sql = 'DELETE FROM lotestrxcab WHERE TxnID = :clave ';
        $stmt = $db->prepare($sql);
        $stmt->bindParam(':clave', $_SESSION['invoice']['TxnID']);
        $stmt->execute();
    } catch (PDOException $e) {
...
function delete_lotestrx($db) {
    $estado = 'INIT';
    try {
        $sql = 'DELETE FROM lotestrx WHERE IDKEY = :clave ';
        $stmt = $db->prepare($sql);
        $stmt->bindParam(':clave', $_SESSION['invoice']['TxnID']);
        $stmt->execute();
    } catch (PDOException $e) {
...
function adiciona_lotestrx($db) {
    $estado = "Procesando adiciona_lotestrx " . $_SESSION['invoicedetail']['ItemRef_FullName'];
    $volante = explode(':', $_SESSION['invoicedetail']['Description']);
    $factura = 'FACTURA';
    $cerrado = 'CERRADO';
    $bodega_nivel1 = getPadreBodega($db, $_SESSION['invoice']['ClassRef_ListID']);
    if ($volante[0] === 'NTV') {
        $_SESSION['volante'] = $volante[1];
    } else {
        try {
            $sql = 'INSERT INTO lotestrx (  TxnLineID, TimeCreated, TimeModified, '
               . 'EditSequence, TxnDate, RefNumber, ItemRef_ListID, '
               . 'ItemRef_FullName, NumeroTrx, FechaTrx, TipoTrx, '
               . 'OrigenTrx, DestinoTrx, OrigenSub, DestinoSub, QtyTrx, IDKEY, Estado) VALUES ( '
               . ':TxnLineID, :TimeCreated, :TimeModified, :EditSequence, '
               . ':TxnDate, :RefNumber, :ItemRef_ListID, :ItemRef_FullName, '
               . ':NumeroTrx, :FechaTrx, :TipoTrx, :OrigenTrx, :DestinoTrx, :OrigenSub, :DestinoSub, :QtyTrx, :IDKEY, :Estado)';
            $stmt = $db->prepare($sql);
            $stmt->bindParam(':TxnLineID', $_SESSION['invoicedetail']['TxnLineID']);
            $stmt->bindParam(':TimeCreated', $_SESSION['invoice']['TimeCreated']);
            $stmt->bindParam(':TimeModified', $_SESSION['invoice']['TimeModified']);
            ...
            $stmt->bindParam(':ItemRef_ListID', $_SESSION['invoicedetail']['ItemRef_ListID']);
            $stmt->bindParam(':ItemRef_FullName', $_SESSION['invoicedetail']['ItemRef_FullName']);
            $stmt->bindParam(':NumeroTrx', $_SESSION['invoice']['EditSequence']);
            $stmt->bindParam(':FechaTrx', $_SESSION['invoice']['TxnDate']);
            $stmt->bindParam(':TipoTrx', $factura);
            $stmt->bindParam(':OrigenTrx', $bodega_nivel1);
            $stmt->bindParam(':DestinoTrx', $_SESSION['invoice']['CustomerRef_ListID']);
            $stmt->bindParam(':OrigenSub', $_SESSION['invoice']['ClassRef_ListID']);
            $stmt->bindParam(':DestinoSub', $_SESSION['invoice']['CustomerRef_ListID']);
            $stmt->bindParam(':QtyTrx', $_SESSION['invoicedetail']['Quantity']);
            $stmt->bindParam(':IDKEY', $_SESSION['invoicedetail']['IDKEY']);
            $stmt->bindParam(':Estado', $cerrado);
            $stmt->execute();
        } catch (PDOException $e) {
...
function getPadreBodega($db, $bodega) {
    $sql = 'SELECT * FROM bodegas WHERE ListID = :bodega';
    $stmt = $db->prepare($sql);
    $stmt->bindParam(':bodega', $bodega);
    $stmt->execute();
    $padre = $stmt->fetch(PDO::FETCH_ASSOC);
    return $padre['ParentRef_ListID'];
}
